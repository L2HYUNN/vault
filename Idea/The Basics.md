...
 TypeScript는 끄거나 킬 수 있는 다양한 strictness type-checking 플래그를 가지고 있다.  CLI에 있는[`strict`](https://www.typescriptlang.org/tsconfig#strict) flag 혹은 [`tsconfig.json`](https://www.typescriptlang.org/docs/handbook/tsconfig-json.html)에 `"strict": true`를 통해 동시에 키거나 끌 수 있다. 알아 두어야 할 가장 중요한 두 가지는 [`noImplicitAny`](https://www.typescriptlang.org/tsconfig#noImplicitAny) 와 [`strictNullChecks`](https://www.typescriptlang.org/tsconfig#strictNullChecks)이다.

## noImplicitAny
일부 장소에서, TypeScript는 타입 추론을 하려는 대신에 가장 관대한 타입인 `any`로 되돌아 간다. 이것이 일어날 수 있는 최악의 일은 아니다 - 결국, `any`로 돌아가는 것은 단지 JavaScript를 경험하는 것이다. 

그러나 `any`를 사용하는 것은 종종 처음에 TypeScript를 사용하고자 한 목적을 잃게한다. 더 많은 타입이 코드에 있으면, 얻을 수 있는 더 많은 유효성 검사와 도구가 있다는 것이고 결국 코드에 더 적은 버그가 있다는 것을 의미한다. 

> [!tip]
> [`noImplicitAny`](https://www.typescriptlang.org/tsconfig#noImplicitAny) flag 를 활성화 하자. 이 flag는 암묵적으로 `any`로 추론되는 타입을 가진 모든 변수애 에러를 발생시킨다.

## strictNullChecks
기본적으로, `null` 그리고 `undefined` 같은 변수들은 아무 다른 타입에 할당할 수 있다. 이것은 일부 코드를 작성하기 쉽게 만들어주지만, `null`과 `undefined`를 다루는 것을 망각하는 것은 세상에 헤아릴 수 없는 버그들의 원인이 된다.

[`strictNullChecks`](https://www.typescriptlang.org/tsconfig#strictNullChecks) flag는 `null`과 `undefined`를 좀 더 분명하게 다룰 수 있게 만들어주며 우리가 `null`과 `undefined`를 다루는 것을 잊는 것에 대한 걱정을 하지 않게 해준다.

> [!summary]
> `strictness`는 TypeScript를 보다 엄격하게 만들어 코드의 안전성을 높여줄 수 있다. 대표적인 strictness 설정에는 `noImplicitAny`와 `strictNullChecks`가 있다.